...
function setDataRow() {
    ...
    if ($('#name').val() != "" && $('#optimist').val() != "" && $('#probably').val() != "" && $('#pesimist').val() != "") {
        if (!issetRow($('#name').val())) {
            var row = [];
            row["name"] = $('#name').val();
            row["precesors"] = $('#precesors').val();

            let op = parseInt($('#optimist').val());
            let pr = parseInt($('#probably').val());
            let pes = parseInt($('#pesimist').val());
            let time = Math.round((op+4*pr+pes)/6);
            let varianza = Math.pow((pes-op)/6,2);
...
function printIniTable() {
    ...
    for (var i = 0; i < dataTable.length; i++) {
        $('#iniTable').append(
            ...
            "<td>" + dataTable[i]["op"] + "</td>" +
            "<td>" + dataTable[i]["pr"] + "</td>" +
            "<td>" + dataTable[i]["pes"] + "</td>" +
            "<td>" + dataTable[i]["time"] + "</td>" +
            "<td>" + dataTable[i]["varianza"] + "</td></tr>"
        );
    }
}
...
function processIniTable() {
    ...
    for (var i = 0; i < route.length - 2; i++) {
        routeString += '<span class="badge badge-pill badge-success bg-success">Actividad '+route[i]+'</span> '+(i<route.length - 3?' -> ':'');
    }
...
function printGraph() {
    ...
    var timeTotal = document.getElementById('timeProyect');
    timeTotal.innerHTML = "Tiempo total del proyecto: " + graph.end.endEarly + " dias";
